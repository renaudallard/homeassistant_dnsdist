...
`;let gt=class extends at{setConfig(t){this._config={...t}}_updateConfig(t,e){if(!this._config)return;const i={...this._config,[t]:e};this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}_getAvailablePrefixes(){if(!this.hass?.states)return[];const t=new Set,e=[/^sensor\.(.+?)_total_queries$/,/^sensor\.(.+?)_uptime$/,/^sensor\.(.+?)_cpu_usage$/,/^sensor\.(.+?)_responses$/,/^sensor\.(.+?)_filter_/,/^sensor\.(.+?)_dynblock_/];for(const i of Object.keys(this.hass.states))for(const s of e){const e=i.match(s);if(e){t.add(e[1]);break}}return Array.from(t).sort()}render(){if(!this.hass||!this._config)return F`<div>Loading...</div>`;const t=this._getAvailablePrefixes();return F`
  ...
  `,t([pt({attribute:!1})],gt.prototype,"hass",void 0),t([ut()],gt.prototype,"_config",void 0),gt=t([lt("dnsdist-card-editor")],gt),window.customCards=window.customCards||[],window.customCards.push({type:"dnsdist-card",name:"dnsdist Card",description:"Display dnsdist DNS server metrics and filtering rules",preview:!0});let _t=mt=class extends at{constructor(){super(...arguments),this._expandedFilters=new Set,this._expandedDynamic=new Set,this._showZeroMatchDynamic=!1,this._showZeroMatchRules=!1,this._showConfirm=!1,this._confirmAction=null}static getConfigElement(){return document.createElement("dnsdist-card-editor")}static getStubConfig(){return{entity_prefix:"dnsdist",show_filters:!0,show_dynamic_rules:!0,show_actions:!0}}setConfig(t){this._config={show_graphs:!1,show_filters:!0,show_dynamic_rules:!0,show_actions:!0,compact:!1,...t,entity_prefix:t.entity_prefix||""}}updated(t){super.updated(t),this._config?.compact?this.setAttribute("compact",""):this.removeAttribute("compact")}connectedCallback(){super.connectedCallback(),this._countdownTimer=setInterval(()=>{this._expandedDynamic.size>0&&this.requestUpdate()},1e3)}disconnectedCallback(){super.disconnectedCallback(),this._countdownTimer&&(clearInterval(this._countdownTimer),this._countdownTimer=void 0)}_getEntityId(t){const e=this._config.entity_prefix,i=mt.METRIC_TO_ENTITY[t]||[t],s=[e,`${e}_${e}`];for(const t of s)for(const e of i){const i=`sensor.${t}_${e}`;if(this.hass?.states?.[i])return i}return`sensor.${e}_${i[0]}`}_getEntity(t){const e=this._getEntityId(t);return e?this.hass?.states?.[e]:void 0}_getEntityValue(t){const e=this._getEntity(t);if(!e)return null;const i=e.state;return"unavailable"===i||"unknown"===i?null:i}_getNumericValue(t){const e=this._getEntityValue(t);if(null===e)return null;const i=parseFloat(String(e));return isNaN(i)?null:i}_formatNumber(t){return null===t?"-":t>=1e6?`${(t/1e6).toFixed(1)}M`:t>=1e3?`${(t/1e3).toFixed(1)}K`:t.toLocaleString()}_getSecurityStatus(){const t=this._getEntityValue("security_status");return t?String(t).toLowerCase():"unknown"}_getSecurityStatusClass(){const t=this._getSecurityStatus();return"ok"===t||"secure"===t?"status-ok":"warning"===t?"status-warning":"critical"===t?"status-critical":"status-unknown"}_getSecurityLabel(){const t=this._getEntity("security_status");return t?.attributes?.status_label||this._getSecurityStatus().toUpperCase()}_getUptimeDisplay(){const t=this._getEntity("uptime");if(!t)return"-";const e=t.attributes?.human_readable;if(e)return e;const i=this._getNumericValue("uptime");if(null===i)return"-";const s=Math.floor(i/86400),r=Math.floor(i%86400/3600),o=Math.floor(i%3600/60);return`${s}d ${r.toString().padStart(2,"0")}h ${o.toString().padStart(2,"0")}m`}_getFilterEntities(){if(!this.hass?.states)return[];const t=this._config.entity_prefix,e=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),i=new RegExp(`^sensor\\.(${e}_)?${e}[_\\s]?filter`,"i"),s=[];for(const e of Object.keys(this.hass.states)){if(!e.startsWith("sensor."))continue;const r=this.hass.states[e],o=r.attributes?.friendly_name||"",n=i.test(e),a=t.toLowerCase().replace(/_/g," "),c=o.toLowerCase().includes(" filter ")&&(o.toLowerCase().startsWith(a)||o.toLowerCase().startsWith(`${a} ${a}`)),l=(void 0!==r.attributes?.action||void 0!==r.attributes?.rule&&void 0!==r.attributes?.enabled)&&(e.toLowerCase().includes(t.toLowerCase())||o.toLowerCase().includes(t.toLowerCase().replace(/_/g," ")));if(!n&&!c&&!l)continue;const d=parseInt(r.state,10),h={matches:isNaN(d)?0:d,id:r.attributes?.id,uuid:r.attributes?.uuid,name:this._extractRuleName(r),action:r.attributes?.action,rule:r.attributes?.rule,type:r.attributes?.type,enabled:r.attributes?.enabled,bypass:r.attributes?.bypass,sources:r.attributes?.sources};s.push({entity:r,rule:h})}return(this._showZeroMatchRules?s:s.filter(({rule:t})=>t.matches>0)).sort((t,e)=>e.rule.matches-t.rule.matches)}_extractRuleName(t){const e=t.attributes?.friendly_name;if(e){const t=e.match(/Filter (.+)$/);if(t)return t[1]}const i=t.entity_id.match(/_filter_(.+)$/);return i?i[1].replace(/_/g," "):"Unknown Rule"}_toggleFilterExpand(t){const e=new Set(this._expandedFilters);e.has(t)?e.delete(t):e.add(t),this._expandedFilters=e}_getDynamicRuleEntities(){if(!this.hass?.states)return[];const t=this._config.entity_prefix,e=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),i=new RegExp(`^sensor\\.(${e}_)?${e}[_\\s]?dynblock`,"i"),s=[];for(const e of Object.keys(this.hass.states)){if(!e.startsWith("sensor."))continue;const r=this.hass.states[e],o=r.attributes?.friendly_name||"",n=i.test(e),a=t.toLowerCase().replace(/_/g," "),c=o.toLowerCase().includes(" dynblock ")&&(o.toLowerCase().startsWith(a)||o.toLowerCase().startsWith(`${a} ${a}`)),l=(void 0!==r.attributes?.network||void 0!==r.attributes?.reason&&void 0!==r.attributes?.seconds)&&(e.toLowerCase().includes(t.toLowerCase())||o.toLowerCase().includes(t.toLowerCase().replace(/_/g," ")));if(!n&&!c&&!l)continue;if("unknown"===r.state||"unavailable"===r.state)continue;const d=parseInt(r.state,10),h=r.attributes?.seconds,p=r.attributes?.network;if(void 0===h||void 0===p)continue;let u=!1;if(h<=0)u=!0;else if(r.last_updated){const t=new Date(r.last_updated).getTime();h-Math.floor((Date.now()-t)/1e3)<=0&&(u=!0)}if(u)continue;const f={blocks:isNaN(d)?0:d,network:p,reason:r.attributes?.reason,action:r.attributes?.action,seconds:h,ebpf:r.attributes?.ebpf,warning:r.attributes?.warning,sources:r.attributes?.sources,last_updated:r.last_updated};s.push({entity:r,rule:f})}return(this._showZeroMatchDynamic?s:s.filter(({rule:t})=>t.blocks>0)).sort((t,e)=>e.rule.blocks-t.rule.blocks)}_extractDynamicRuleName(t){const e=t.attributes?.friendly_name;if(e){const t=e.match(/Dynblock (.+)$/);if(t)return t[1]}const i=t.attributes?.network;if(i)return i;const s=t.entity_id.match(/_dynblock_(.+)$/);return s?s[1].replace(/_/g,"."):"Unknown"}_toggleDynamicExpand(t){const e=new Set(this._expandedDynamic);e.has(t)?e.delete(t):e.add(t),this._expandedDynamic=e}_formatTimeRemaining(t,e){if(void 0===t||t<=0)return"-";let i=t;if(e){const s=new Date(e).getTime(),r=Date.now(),o=Math.floor((r-s)/1e3);i=Math.max(0,t-o)}if(i<=0)return"Expiring...";const s=Math.floor(i/60),r=Math.floor(i%60);return s>0?`${s}m ${r}s`:`${r}s`}_getActionClass(t){if(!t)return"";const e=t.toLowerCase();return e.includes("drop")||e.includes("refuse")?"drop":e.includes("allow")||e.includes("pool")?"allow":""}async _clearCache(){const t=this._config.entity_prefix,e=`button.${t}_clear_cache`;this.hass.states[e]?await this.hass.callService("button","press",{entity_id:e}):await this.hass.callService("dnsdist","clear_cache",{host:t})}_showClearCacheConfirm(){this._confirmAction=()=>this._clearCache(),this._showConfirm=!0}_confirmDialogAction(){this._confirmAction&&this._confirmAction(),this._hideConfirm()}_hideConfirm(){this._showConfirm=!1,this._confirmAction=null}_toggleShowZeroMatchDynamic(){this._showZeroMatchDynamic=!this._showZeroMatchDynamic}_toggleShowZeroMatchRules(){this._showZeroMatchRules=!this._showZeroMatchRules}_renderGaugeSvg(t,e){return F`
      <svg class="gauge-svg" viewBox="0 0 100 100">
        <path d="M 21.72 78.28 A 40 40 0 0 1 10.77 57.80" fill="none" stroke="${e[0]}" stroke-width="8"/>
        <path d="M 10.77 57.80 A 40 40 0 0 1 13.04 34.69" fill="none" stroke="${e[1]}" stroke-width="8"/>
        <path d="M 13.04 34.69 A 40 40 0 0 1 27.78 16.74" fill="none" stroke="${e[2]}" stroke-width="8"/>
        <path d="M 27.78 16.74 A 40 40 0 0 1 50.00 10.00" fill="none" stroke="${e[3]}" stroke-width="8"/>
        <path d="M 50.00 10.00 A 40 40 0 0 1 72.22 16.74" fill="none" stroke="${e[4]}" stroke-width="8"/>
        <path d="M 72.22 16.74 A 40 40 0 0 1 86.96 34.69" fill="none" stroke="${e[5]}" stroke-width="8"/>
        <path d="M 86.96 34.69 A 40 40 0 0 1 89.23 57.80" fill="none" stroke="${e[6]}" stroke-width="8"/>
        <path d="M 89.23 57.80 A 40 40 0 0 1 78.28 78.28" fill="none" stroke="${e[7]}" stroke-width="8"/>
        <g class="gauge-needle-group" style="transform: rotate(${t}deg); transform-origin: 50px 50px;">
          <line class="gauge-needle" x1="50" y1="50" x2="50" y2="16" />
        </g>
        <circle class="gauge-pivot" cx="50" cy="50" r="4" />
      </svg>
    `}_renderGaugeInfo(t,e,i){return F`
      <div class="gauge-info gauge-info-${i}">
        <div class="gauge-value">${null!==t?`${t.toFixed(0)}%`:"-"}</div>
        <div class="gauge-label">${e}</div>
      </div>
    `}_renderGaugeGreenToRed(t,e,i){const s=(null!==t?Math.min(100,Math.max(0,t)):0)/100*270-135;return F`
      <div class="gauge">
        ${"left"===i?this._renderGaugeInfo(t,e,i):""}
        ${this._renderGaugeSvg(s,["#4caf50","#8bc34a","#cddc39","#ffeb3b","#ffc107","#ff9800","#ff5722","#f44336"])}
        ${"right"===i?this._renderGaugeInfo(t,e,i):""}
      </div>
    `}_renderGaugeRedToGreen(t,e,i){const s=(null!==t?Math.min(100,Math.max(0,t)):0)/100*270-135;return F`
      <div class="gauge">
        ${"left"===i?this._renderGaugeInfo(t,e,i):""}
        ${this._renderGaugeSvg(s,["#f44336","#ff5722","#ff9800","#ffc107","#ffeb3b","#cddc39","#8bc34a","#4caf50"])}
        ${"right"===i?this._renderGaugeInfo(t,e,i):""}
      </div>
    ...
    `}_renderFilterItem(t,e){const i=this._expandedFilters.has(t);return F`
      ...
      >
        <div class="filter-main">
          ...
          ${e.action?F`<span class="filter-action ${this._getActionClass(e.action)}">${e.action}</span>`:G}
        </div>
              ...
              <div class="filter-details">
                ...
                ${void 0!==e.seconds&&e.seconds>0?F`
                      <div class="filter-detail-row">
                        ...
                        <span class="filter-detail-value">${this._formatTimeRemaining(e.seconds,e.last_updated)}</span>
                      </div>
                    `:G}
                ${void 0!==e.ebpf?F`
                      <div class="filter-detail-row">
    ...
    `:G}render(){if(!this._config||!this.hass)return F`<ha-card><div class="loading">Loading...</div></ha-card>`;if(!this._config.entity_prefix)return F`
        <ha-card>
      ...
      <ha-card>
        ...
        <div class="gauge-container">
          ${this._renderGaugeGreenToRed(e,"CPU","left")}
          ${this._renderGaugeRedToGreen(i,"Cache Hit","right")}
        </div>
        ...
        ${this._config.show_actions?F`
              <div class="actions-container">
                ...
                ${this._config.show_filters?F`
                    ...
                    `:G}
                ${this._config.show_dynamic_rules?F`
                      <button
