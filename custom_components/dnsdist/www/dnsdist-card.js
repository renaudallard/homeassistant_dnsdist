...
`;let gt=class extends at{setConfig(t){this._config={...t}}_updateConfig(t,e){if(!this._config)return;const i={...this._config,[t]:e};this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i},bubbles:!0,composed:!0}))}_getAvailablePrefixes(){if(!this.hass?.states)return[];const t=new Set,e=[/^sensor\.(.+?)_total_queries$/,/^sensor\.(.+?)_uptime$/,/^sensor\.(.+?)_cpu_usage$/,/^sensor\.(.+?)_responses$/,/^sensor\.(.+?)_filter_/];for(const i of Object.keys(this.hass.states))for(const s of e){const e=i.match(s);if(e){t.add(e[1]);break}}return Array.from(t).sort()}render(){if(!this.hass||!this._config)return F`<div>Loading...</div>`;const t=this._getAvailablePrefixes();return F`
  ...
  `,t([pt({attribute:!1})],gt.prototype,"hass",void 0),t([ut()],gt.prototype,"_config",void 0),gt=t([dt("dnsdist-card-editor")],gt),window.customCards=window.customCards||[],window.customCards.push({type:"dnsdist-card",name:"dnsdist Card",description:"Display dnsdist DNS server metrics and filtering rules",preview:!0});let _t=mt=class extends at{constructor(){super(...arguments),this._expandedFilters=new Set,this._showConfirm=!1,this._confirmAction=null}static getConfigElement(){return document.createElement("dnsdist-card-editor")}static getStubConfig(){return{entity_prefix:"dnsdist",show_filters:!0,show_actions:!0}}setConfig(t){this._config={show_graphs:!1,show_filters:!0,show_actions:!0,compact:!1,...t,entity_prefix:t.entity_prefix||""}}updated(t){super.updated(t),this._config?.compact?this.setAttribute("compact",""):this.removeAttribute("compact")}_getEntityId(t){const e=this._config.entity_prefix,i=mt.METRIC_TO_ENTITY[t]||[t],s=[e,`${e}_${e}`];for(const t of s)for(const e of i){const i=`sensor.${t}_${e}`;if(this.hass?.states?.[i])return i}return`sensor.${e}_${i[0]}`}_getEntity(t){const e=this._getEntityId(t);return e?this.hass?.states?.[e]:void 0}_getEntityValue(t){const e=this._getEntity(t);if(!e)return null;const i=e.state;return"unavailable"===i||"unknown"===i?null:i}_getNumericValue(t){const e=this._getEntityValue(t);if(null===e)return null;const i=parseFloat(String(e));return isNaN(i)?null:i}_formatNumber(t){return null===t?"-":t>=1e6?`${(t/1e6).toFixed(1)}M`:t>=1e3?`${(t/1e3).toFixed(1)}K`:t.toLocaleString()}_getSecurityStatus(){const t=this._getEntityValue("security_status");return t?String(t).toLowerCase():"unknown"}_getSecurityStatusClass(){const t=this._getSecurityStatus();return"ok"===t||"secure"===t?"status-ok":"warning"===t?"status-warning":"critical"===t?"status-critical":"status-unknown"}_getSecurityLabel(){const t=this._getEntity("security_status");return t?.attributes?.status_label||this._getSecurityStatus().toUpperCase()}_getUptimeDisplay(){const t=this._getEntity("uptime");if(!t)return"-";const e=t.attributes?.human_readable;if(e)return e;const i=this._getNumericValue("uptime");if(null===i)return"-";const s=Math.floor(i/86400),r=Math.floor(i%86400/3600),o=Math.floor(i%3600/60);return`${s}d ${r.toString().padStart(2,"0")}h ${o.toString().padStart(2,"0")}m`}_getFilterEntities(){if(!this.hass?.states)return[];const t=this._config.entity_prefix,e=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),i=new RegExp(`^sensor\\.(${e}_)?${e}[_\\s]?filter`,"i"),s=[];for(const e of Object.keys(this.hass.states)){if(!e.startsWith("sensor."))continue;const r=this.hass.states[e],o=r.attributes?.friendly_name||"",n=i.test(e),a=t.toLowerCase().replace(/_/g," "),c=o.toLowerCase().includes(" filter ")&&(o.toLowerCase().startsWith(a)||o.toLowerCase().startsWith(`${a} ${a}`)),d=(void 0!==r.attributes?.action||void 0!==r.attributes?.rule&&void 0!==r.attributes?.enabled)&&(e.toLowerCase().includes(t.toLowerCase())||o.toLowerCase().includes(t.toLowerCase().replace(/_/g," ")));if(!n&&!c&&!d)continue;const l=parseInt(r.state,10),h={matches:isNaN(l)?0:l,id:r.attributes?.id,uuid:r.attributes?.uuid,name:this._extractRuleName(r),action:r.attributes?.action,rule:r.attributes?.rule,type:r.attributes?.type,enabled:r.attributes?.enabled,bypass:r.attributes?.bypass,sources:r.attributes?.sources};s.push({entity:r,rule:h})}return s.sort((t,e)=>e.rule.matches-t.rule.matches)}_extractRuleName(t){const e=t.attributes?.friendly_name;if(e){const t=e.match(/Filter (.+)$/);if(t)return t[1]}const i=t.entity_id.match(/_filter_(.+)$/);return i?i[1].replace(/_/g," "):"Unknown Rule"}_toggleFilterExpand(t){const e=new Set(this._expandedFilters);e.has(t)?e.delete(t):e.add(t),this._expandedFilters=e}_getActionClass(t){if(!t)return"";const e=t.toLowerCase();return e.includes("drop")||e.includes("refuse")?"drop":e.includes("allow")||e.includes("pool")?"allow":""}async _clearCache(){const t=this._config.entity_prefix,e=`button.${t}_clear_cache`;this.hass.states[e]?await this.hass.callService("button","press",{entity_id:e}):await this.hass.callService("dnsdist","clear_cache",{host:t})}_showClearCacheConfirm(){this._confirmAction=()=>this._clearCache(),this._showConfirm=!0}_confirmDialogAction(){this._confirmAction&&this._confirmAction(),this._hideConfirm()}_hideConfirm(){this._showConfirm=!1,this._confirmAction=null}_renderGauge(t,e,i){const s=null!==t?Math.min(100,Math.max(0,t)):0;return F`
    ...
    `}_renderFilterItem(t,e){const i=this._expandedFilters.has(t);return F`
      ...
      >
        <div class="filter-main">
          ...
          ${e.action?F`<span class="filter-action ${this._getActionClass(e.action)}">${e.action}</span>`:B}
        </div>
    ...
    `:B}render(){if(!this._config||!this.hass)return F`<ha-card><div class="loading">Loading...</div></ha-card>`;if(!this._config.entity_prefix)return F`
        <ha-card>
      ...
      `;const t=this._config.title||this._config.entity_prefix,e=this._getNumericValue("cpu"),i=this._getNumericValue("cache_hit_rate"),s=this._config.show_filters?this._getFilterEntities():[];return F`
      <ha-card>
        <!-- Header -->
        ...
        <div class="gauge-container">
          ${this._renderGauge(e,"CPU")}
          ${this._renderGauge(i,"Cache Hit")}
        </div>
