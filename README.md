...
> **Rate sensors need runway:** `req_per_hour` stabilizes after the first hour. `req_per_day` is extrapolated from available history until 24 hours of data is collected (e.g., 1 hour of data is scaled to estimate the daily rate), then switches to actual measured values.
...
- Extrapolate daily request rate from available history when less than 24 hours of data is collected
